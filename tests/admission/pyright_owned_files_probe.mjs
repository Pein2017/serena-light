...
export function rewriteBootstrapSource(source) {
  ...
  return source.replace(
    BOOTSTRAP_MARKER,
    `globalThis.${WEBPACK_REQUIRE_GLOBAL}=o})();`,
  );
}
...
function locatePinnedPackage() {
  ...
  return { executable, runtimeRoot, packageRoot };
}
...
function selectedProjectEvidence(service) {
  ...
  if (!configOptions || typeof configOptions !== "object") {
    throw new ProbeError(
      "PYRIGHT_CONFIG_OPTIONS_INVALID",
      "AnalyzerService.getConfigOptions did not return native config options",
    );
  }
  ...
  if (!configFileSource || typeof configFileSource.getFilePath !== "function") {
    throw new ProbeError(
      "PYRIGHT_CONFIG_OPTIONS_INVALID",
      "native configFileSource is not a Pyright file URI",
    );
  }
  ...
  if (typeof configPath !== "string" || !isAbsolute(configPath) || configPath.includes("\0")) {
    throw new ProbeError(
      "PYRIGHT_CONFIG_OPTIONS_INVALID",
      "native configFileSource did not produce a safe absolute path",
    );
  }
  return {
  ...
  try {
    ...
    AnalyzerService.prototype.setOptions = function nativeSetOptionsAndCapture(options) {
      try {
        ...
        if (captured) {
          throw new ProbeError("PYRIGHT_SET_OPTIONS_REPEATED", "native CLI invoked AnalyzerService.setOptions more than once");
        }
        captured = true;
        const ownedFiles = ownedPathList(this);
        const project = selectedProjectEvidence(this);
        ...
        const report = {
          ...
          bundle: {
            ...
            pyright_internal_js: {
              path: pyrightInternalJsPath,
              sha256: evidence.pyrightInternalJsSha256,
            },
          },
