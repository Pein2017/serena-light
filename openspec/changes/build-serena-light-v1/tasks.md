...
- [x] 15.8f Disposition the exact-`483e7a4` Sol-xhigh HOLD and Opus-max P3 findings: use guarded byte-sensitive freshness for same-stat source/config rewrites, fail closed before scan commit on unstable observations without breaking stable deletion/symlink transitions, and preserve the typed Python-liveness error before reading retry/generation details.
