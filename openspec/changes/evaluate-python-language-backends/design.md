...
Read-only tasks use immutable snapshots or verified manifests. Full content digests cover the lexical trust-inventory closure and declared fixture paths. All other in-scope paths, including large ignored subtrees such as `research-probes/model_cache`, receive a metadata-only path/type/symlink-target/size/`mtime_ns`/inode sweep; content is hashed only when metadata changes. Controlled diagnostics, concurrent-change, and guarded-edit tasks use disposable copies under a temporary evaluation root. The manifest distinguishes declared fixture mutations from unexpected backend writes. Missing roots or an inability to freeze dirty state stops the run.
...
- **Real dirty worktrees can change during setup** -> freeze bounded trust/fixture content manifests plus remainder metadata and use snapshots; fail if the source changes while being frozen.
