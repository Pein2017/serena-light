...
- [ ] 6.3 Run real-daemon/fresh-client Codex, Claude Code, and CC Agent lifecycle acceptance on representative Python `/data/CoordExp` and JS/TS `/data/CoordExp/external/codexUI` roots, including two build daemons with live old-build leases; do not repeat lexical Unicode or transformers-scope matrices unchanged by this policy.
